...
{
  const struct dof *dof = (const struct dof *) dofc->ptr;
  const struct dof *dof_end = dof+cn;
  ...
  struct array *dofa, buffer *buf, const struct comm *comm)
{
  ...
  array_init(struct dof,dofa,cn);
  dofa->n = cn;
  dof = (struct dof *) dofa->ptr;
  buffer_reserve(buf,cn*sizeof(ulong));
  bid = (ulong *) buf->ptr;
